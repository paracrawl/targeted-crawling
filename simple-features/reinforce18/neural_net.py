...
class Qnetwork():
    def __init__(self, params):
        ...
        # layer 3
        self.WlinkSpecific3 = tf.Variable(tf.random_uniform([params.linkDim, params.linkDim], 0, 0.01))
        self.blinkSpecific3 = tf.Variable(tf.random_uniform([1, params.linkDim], 0, 0.01))

        self.linkSpecific = tf.matmul(self.linkSpecific, self.WlinkSpecific3)
        self.linkSpecific = tf.add(self.linkSpecific, self.blinkSpecific3)
        print("self.linkSpecific6", self.linkSpecific.shape)

        self.linkSpecific = tf.reshape(self.linkSpecific, [self.batchSize, self.params.NUM_ACTIONS, params.linkDim])
        #self.linkSpecific = tf.nn.relu(self.linkSpecific)
        print("self.linkSpecific7", self.linkSpecific.shape)
